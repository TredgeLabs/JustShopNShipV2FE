...
const OrderCorrection: React.FC = () => {
  ...
  const handleItemEdit = (itemId: number, field: string, value: string | number) => {
    ...
    setOrderData(prev => ({
      ...prev!,
      items: (prev!.local_order_items ?? []).map(item =>
        item.id === itemId
          ? { ...item, [field]: value }
          : item
      )
  ...
  const deleteItem = (itemId: number) => {
    ...
    if (confirmed) {
      setOrderData(prev => ({
        ...prev!,
        items: (prev!.local_order_items ?? []).filter(item => item.id !== itemId)
      }));
    }
  };
  ...
  const handleAddNewItem = () => {
    ...
    setOrderData(prev => ({
      ...prev!,
      items: [...(prev!.local_order_items ?? []), newItemData]
    }));
  ...
  const calculateCurrentTotal = () => {
    ...
    return (orderData.local_order_items ?? []).reduce((total, item) => total + (item.final_price * item.quantity), 0);
  };
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="text-sm text-gray-600">
                  Order ID: <span className="font-medium">{orderData.id}</span>
                </div>
              </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-gray-900">₹{orderData.total_price.toLocaleString()}</span>
                </div>
